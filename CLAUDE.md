...
```bash
# Grid-search temperature tuning with injected Runner + baseline Judge
cd AutoTemp && GOMAXPROCS=2 nice -n 19 go test -count=1 -race -v ./pkg/client
```
Expect: PASS; baseline runner/judge seeded, grid search over `[0.1, 0.4, 0.7, 1.0]` picks a best temperature and summary.
