...
// AutoTemp submodule. It is the CONST-046-compliant indirection between
// production code in pkg/client and pkg/types and the locale bundles
// under bundles/.
//
...
// default), and the AutoTemp submodule never reaches into a parent
// project to discover its own catalogue.
//
// Usage:
//
//	t := i18n.NoopTranslator{} // production default — returns key verbatim
//	msg := t.T("autotemp_run_summary", map[string]any{
//	    "temperature": 0.7,
//	    "score":       0.93,
//	    "candidates":  4,
//	})
...
// bundles/active.en.yaml + locale overrides; the AutoTemp submodule
// remains project-not-aware.
//
// Scope note (round 132 §11.4 kickoff): AutoTemp's source surface is
// programmatic — `RunResult.Summary`, `BenchmarkResult.Summary`, and the
// `ErrBaselineRunnerNotConfigured` sentinel are consumed by other Go
// packages, not displayed to end users. The bundle below seeds
// locale-aware companion keys (`autotemp_*`) that future UI/CLI
// consumers can resolve when surfacing run results, benchmark summaries,
// and runner-misconfiguration diagnostics to end users — without
// forcing a breaking change to the existing programmatic contracts.
package i18n
...
// surface shape ("autotemp_run_summary") rather than an empty string —
